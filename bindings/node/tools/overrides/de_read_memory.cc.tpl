...
// SPDX-License-Identifier: AGPL-3.0-or-later
  ...
  if (args.Length() < 2) {
    Napi::TypeError::New(env, "de_read_memory: expected (formatId, buffer)").ThrowAsJavaScriptException();
    return env.Undefined();
  }

  if (!args.Get(0u).IsString()) {
    Napi::TypeError::New(env, "de_read_memory: formatId must be a string").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  std::string format_id = args.Get(0u).ToString().Utf8Value();
  ...
  if (vv.IsBuffer()) {
    ...
    if (array.TypedArrayType() != napi_uint8_array && array.TypedArrayType() != napi_uint8_clamped_array) {
      Napi::TypeError::New(env, "de_read_memory: typed array must be Uint8Array").ThrowAsJavaScriptException();
      return env.Undefined();
    }
    ...
    Napi::TypeError::New(env, "de_read_memory: expected Buffer, Uint8Array, or ArrayBuffer").ThrowAsJavaScriptException();
    return env.Undefined();
  }

  occtl_graph_t* out_graph = nullptr;
  occtl_node_id_t out_root = OCCTL_NODE_ID_INVALID;
  occtl_status_t status = occtl_de_read_memory(format_id.c_str(), data, size, &out_graph, &out_root);
  if (status != OCCTL_OK) {
