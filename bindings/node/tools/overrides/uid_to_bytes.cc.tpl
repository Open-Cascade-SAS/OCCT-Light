...
// SPDX-License-Identifier: AGPL-3.0-or-later
  ...
  if (args.Length() < 1) {
    Napi::TypeError::New(env, "uid_to_bytes: expected uid").ThrowAsJavaScriptException();
  ...
  if (value.IsBigInt()) {
    ...
    Napi::TypeError::New(env, "uid_to_bytes: uid must be a BigInt or {bits}").ThrowAsJavaScriptException();
    return env.Undefined();
  }

  occtl_uid_t uid = { bits };
  uint8_t out_bytes[OCCTL_UID_WIRE_SIZE] = {};
  occtl_status_t status = occtl_uid_to_bytes(uid, out_bytes);
  if (status != OCCTL_OK) {
    ThrowFromStatus(env, status);
    return env.Undefined();
  }
  return Napi::Buffer<uint8_t>::Copy(env, out_bytes, OCCTL_UID_WIRE_SIZE);
