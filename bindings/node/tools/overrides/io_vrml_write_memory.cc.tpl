...
// SPDX-License-Identifier: AGPL-3.0-or-later
  ...
  if (args.Length() < 2) {
    Napi::TypeError::New(env, "io_vrml_write_memory: expected (graph, root[, options])").ThrowAsJavaScriptException();
  ...
  occtl_io_vrml_write_options_t options = OCCTL_IO_VRML_WRITE_OPTIONS_INIT;
  const occtl_io_vrml_write_options_t* options_ptr = nullptr;
  if (args.Length() >= 3 && args.Get(2u).IsObject()) {
    ...
    if (obj.Has("writer_version")) {
      options.writer_version =
        (occtl_io_vrml_writer_version_t)obj.Get("writer_version").ToNumber().Int64Value();
    }
    if (obj.Has("representation")) {
      options.representation =
        (occtl_io_vrml_representation_t)obj.Get("representation").ToNumber().Int64Value();
    }
  }

  size_t required = 0;
  occtl_status_t status =
    occtl_io_vrml_write_memory(graph, root, options_ptr, nullptr, 0, &required);
  if (status != OCCTL_OK) {
  ...
  status = occtl_io_vrml_write_memory(graph,
                                      root,
                                      options_ptr,
                                      buffer.Data(),
                                      buffer.Length(),
