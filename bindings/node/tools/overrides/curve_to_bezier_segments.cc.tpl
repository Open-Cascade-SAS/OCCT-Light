...
// SPDX-License-Identifier: AGPL-3.0-or-later
  ...
  if (args.Length() < 1) {
    Napi::TypeError::New(env, "curve_to_bezier_segments: expected (graph, curveId[, options])").ThrowAsJavaScriptException();
  ...
  if (!graph) {
    Napi::Error::New(env, "curve_to_bezier_segments: first argument must be a Graph").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  ...
  occtl_status_t status =
    occtl_curve_to_bezier_segments(graph, curve_id, options_ptr, &segments, &count);
  if (status != OCCTL_OK) {
