...
"""
Run Go commands with feature-aware OCCT-Light build flags from OCCTLFeatures.json.

Usage:
  python3 tools/go_with_features.py /path/to/OCCTLFeatures.json -- go test ./...
  python3 tools/go_with_features.py /path/to/OCCTLFeatures.json -- go build ./...
"""
...
def parse_args(argv: Sequence[str]) -> tuple[pathlib.Path, list[str]]:
    if "--" not in argv:
        die("usage: go_with_features.py /path/to/OCCTLFeatures.json -- go <args>")
    split = argv.index("--")
    if split != 1:
        die("first argument must be the OCCTLFeatures.json path")
    if split + 1 >= len(argv):
        die("missing go command after '--'")
    manifest = pathlib.Path(argv[0]).resolve()
    cmd = list(argv[split + 1 :])
    return manifest, cmd


def main() -> None:
    manifest_path, cmd = parse_args(sys.argv[1:])
    if not manifest_path.is_file():
        die(f"manifest not found: {manifest_path}")

    data = json.loads(manifest_path.read_text(encoding="utf-8"))
    features = [str(x) for x in (data.get("binding_features") or [])]
    ...
    if repo_root is None:
        die("could not infer repository root from manifest path")
    ...
    tags = [FEATURE_TO_GO_TAG[f] for f in features if f in FEATURE_TO_GO_TAG]

    env = os.environ.copy()
    env["CGO_CFLAGS"] = " ".join([f"-I{include_dir}", *macros])
    env["CGO_LDFLAGS"] = " ".join([f"-L{lib_dir}", f"-Wl,-rpath,{lib_dir}", f"-l{library_name}"])
    if tags:
        env["GOFLAGS"] = f"-tags={','.join(tags)}"
    elif "GOFLAGS" in env:
        del env["GOFLAGS"]
