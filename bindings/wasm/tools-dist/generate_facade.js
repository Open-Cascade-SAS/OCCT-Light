...
function moduleForHeader(h) {
    return HEADER_TO_MODULE[h];
}
function validateKnownHeaders(abi) {
    const headers = new Set(abi.headers ?? []);
    for (const fn of abi.functions)
        headers.add(fn.header);
...
const ARRAY_RE = /^(.+?)\[(\d+)\]$/;
function resolveTypeSize(typeName, types, resolved) {
...
function computeStructSizeAlign(structType, types, resolved) {
...
// =====================================================================
function isVoidReturn(fn) {
    return fn.return_type === "void";
}
function isPointerReturn(fn) {
    return fn.return_type.includes("*");
}
function paramCType(p) {
    return p.type;
}
function collectFunctionPointerAliases(abi) {
...
function emitFnEmbindWrapper(fn, functionPointerAliases) {
    const lines = [];
    const args = [];
    const fwdArgs = [];
...
function emitRawCc(abi) {
    ...
    for (const mod of MODULES) {
        ...
        const enums = abi.types.filter((t) => t.kind === "enum" && moduleForHeader(t.header) === mod && t.name.endsWith("_t"));
        if (fns.length === 0 && enums.length === 0)
            continue;
...
function cTypeToTsRaw(ctype, pointerDepth) {
    ...
    if (t === "int" || t === "int32_t" || t === "uint32_t" || t === "uint8_t" || t === "int8_t" ||
        t === "uint16_t" || t === "int16_t" || t === "size_t" || t === "ptrdiff_t")
        return "number";
    if (t === "int64_t" || t === "uint64_t")
        return "bigint";
    if (t === "char")
        return "number";
...
function emitGeneratedTs(abi, mod) {
    const fns = abi.functions.filter((f) => moduleForHeader(f.header) === mod);
    const enums = abi.types.filter((t) => t.kind === "enum" && moduleForHeader(t.header) === mod && t.name.endsWith("_t"));
    ...
    if (constants.some((c) => typeof c.value === "number"))
        out.push("");
    ...
    for (const fn of fns) {
        ...
        out.push(`    ${snakeToCamel(fn.name)}: (${fn.params.map((p) => snakeToCamel(p.name)).join(", ") || ""}) => _getFn(mod, "${fn.name}")(${params}),`);
    }
    out.push("  };");
    out.push("}");
    out.push("");
